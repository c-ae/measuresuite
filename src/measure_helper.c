...
static int init_json(struct measuresuite *ms) {
  ...
  // ceil(log10(results_c[0])));
  // assuming, one batch takes less that 999 cycles, which needs three
  // chars 7 chars for '[-1,,],', times 2 for (a or b) and c -> two measurements
  // + 2 for each in runorder
  int char_per_run = 2 + 7 + (2 * 3 * ceil(log10(ms->batch_size)));
  // one measurement is roughly (eval=10) "[-1,9990,9990],"*/
  ms->json_len = 1025 + char_per_run * ms->num_batches;
